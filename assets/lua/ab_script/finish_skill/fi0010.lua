-- 4027630: UR_超サイヤ人3孫悟空_フィニッシュ：龍拳
-- battle_301275
-- fi0010
...
if (_IS_PLAYER_SIDE_ == 1) then
    ...
    if (_IS_SKIP_ == 1) then
       ...
       if(_IS_DODGE_ == 1) then
           skipFrame(0, spep_0 + 346 -12 -1 );        -- スキップかつ回避された時のスキップ先フレーム指定
           setupMovie(spep_0 + 346 -12, SP_01, spep_0 + 346 -12 -1 + 2, 1);
       else
           skipFrame(0, spep_0 + 752 );      -- スキップ先フレーム指定
           setupMovie(spep_0 + 752, SP_01, spep_0 + 752 -1 + 2, 1);
   end
    else 
...
-- BGMの変更 (ゼロフレーム目から0秒かけて次のBGMに切り替える
changeBgm( 0 , 0 );
