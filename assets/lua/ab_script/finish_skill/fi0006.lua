...
if (_IS_PLAYER_SIDE_ == 1) then
    ...
    if (_IS_SKIP_ == 1) then
       ...
       if(_IS_DODGE_ == 1) then
           skipFrame(0, spep_0 + 208 -12);        -- スキップかつ回避された時のスキップ先フレーム指定
       else
           skipFrame(0, spep_0 + 484);      -- スキップ先フレーム指定
...
--画面遷移
SE005 = playSeVer2( spep_0 + 80, 1072, "", 0, 0, 0, -1);

-- ** ボイス ** --
--「おまえの!!!!　負けだーーーー!!!!」
playVoice( spep_0 + 18, 628 );
setVoiceVolume( spep_0 + 18, 628, 126 );
...
if(_IS_DODGE_ == 1) then
    ...
    setVoiceVolume( SP_dodge - 12, 628, 0 );
...
SE020 = playSeVer2( spep_0 + 502, 1159, "", 0, 0, 0, -1);
...
-- ** ボイス ** --
--「へ…へへ…!!!」
playVoice( spep_0 + 746, 629 );
setVoiceVolume( spep_0 + 746, 629, 126 );
