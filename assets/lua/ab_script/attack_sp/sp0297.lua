--4006521:UR_スーパーベビー2(大猿ベビー)_必殺技：スーパーギャリック砲
--sp_effect_a1_00002
--sp0297
...
SE001X = playSe(SP_start+ 260, SE_07);
...
SE002X = playSe(SP_start+ 300, SE_07);
...
-- カメハメハ発射(110F)
------------------------------------------------------
changeAnime(SP_start+ 260 + add_f, 0, 31);                                    -- かめはめ発射ポーズ
setMoveKey(SP_start+  259 + add_f,    0,      0,  0,   0);
setMoveKey(SP_start+  260 + add_f,    0, -400,  0,   0);
setMoveKey(SP_start+  270 + add_f,    0,      0,  0,   0);
setMoveKey(SP_start+  290 + add_f,    0,   -30,  0,   20);

kamehame_beam = entryEffectLife(SP_start+ 260 + add_f, SP_02, 109, 0x40,  0,  300,  550, 170);   -- 伸びるかめはめ波
setEffScaleKey(SP_start+ 260 + add_f, kamehame_beam, 1.5, 1.5);


SE001X = playSe(SP_start+ 260 + add_f, SE_07);

setEffMoveKey(SP_start+ 290 + add_f,kamehame_beam,400,150 );--221
setEffMoveKey(SP_start+ 300 + add_f,kamehame_beam,230,80 );--231
setEffMoveKey(SP_start+ 305 + add_f,kamehame_beam,130,70 );--236

--spname = entryEffect(SP_start+ 260, 1508,  0,  -1,  0,  0,  0);               -- かめはめ波文字
--setEffReplaceTexture(spname, 7, 4);                                  -- 技名テクスチャ差し替え

-- 途中からカメハメハが伸びる
setMoveKey(SP_start+  300 + add_f,    0,   -150,  0,   50);
setMoveKey(SP_start+  305 + add_f,    0,   -600,  0,   80);


entryFade(SP_start+ 365 + add_f, 3, 2, 5, fcolor_r, fcolor_g, fcolor_b, 255);     -- white fade

-- ** エフェクト等 ** --
entryFadeBg(SP_start+ 260 + add_f, 0, 109, 0, 255, 255, 255, 255);       -- ベース暗め　背景
entryFadeBg(SP_start+ 260 + add_f, 0, 40, 0, 230, 230, 230, 255);    -- 最初だけ明るい　背景
--entryFadeBg( 300, 0, 2, 8, 100, 140, 250, 250);     -- 青い　背景
entryFadeBg(SP_start+ 300 + add_f, 0, 2, 8, 0, 0, 0, 250);     -- 青い　背景

sen2 = entryEffectLife(SP_start+ 260 + add_f, 921, 109, 0x80,  -1,  0,  0,  0); -- 流線
setEffRotateKey(SP_start+260 + add_f, sen2, 190);
setShakeChara(SP_start+ 305 + add_f, 0, 54, 50);
setEffScaleKey(SP_start+  260 + add_f, sen2, 1.3, 1.3);

shuchusen = entryEffectLife(SP_start+ 260 + add_f, 906, 49, 0x00,  -1, 0,  0,  0);   -- 集中線
setEffScaleKey(SP_start+ 260 + add_f, shuchusen, 1.0, 1.0);
setEffScaleKey(SP_start+ 300 + add_f, shuchusen, 1.0, 1.0);
setEffScaleKey(SP_start+ 309 + add_f, shuchusen, 2.0, 2.0);

-- 書き文字エントリー
ct = entryEffectLife(SP_start+ 268 + add_f, 10012, 32, 0, -1, 0, 200, 300); -- ズオッ
setEffShake(SP_start+268 + add_f, ct, 32, 5);
setEffAlphaKey(SP_start+268 + add_f, ct, 255);
setEffAlphaKey(SP_start+290 + add_f, ct, 255);
setEffAlphaKey(SP_start+300 + add_f, ct, 0);
setEffScaleKey(SP_start+268 + add_f, ct, 0.0, 0.0);
setEffScaleKey(SP_start+272 + add_f, ct, 1.3, 1.3);
setEffScaleKey(SP_start+292 + add_f, ct, 1.3, 1.3);
setEffScaleKey(SP_start+300 + add_f, ct, 6.0, 6.0);
SE002X = playSe(SP_start+ 300 + add_f, SE_07);
entryFade(SP_start+ 355 + add_f, 10,  5, 10, fcolor_r, fcolor_g, fcolor_b, 255);     -- white fade
------------------------------------------------------
-- かめはめは迫る(100F)
------------------------------------------------------
setDisp(SP_start+ 370 + add_f, 0, 0);
setDisp(SP_start+ 370 + add_f, 1, 1);
changeAnime(SP_start+ 370 + add_f, 1, 104);                        -- ガード
setMoveKey(SP_start+  369 + add_f,    1,  235,  0,   0);
setMoveKey(SP_start+  370 + add_f,    1,  235,  0,   0);
setMoveKey(SP_start+  371 + add_f,    1,  235,  0,   0);
setScaleKey(SP_start+ 369 + add_f,    1,  0.7, 0.7);
setScaleKey(SP_start+ 370 + add_f,    1,  0.7, 0.7);

setMoveKey(SP_start+  400 + add_f,    1,   120,  0,   0);
setScaleKey(SP_start+  400 + add_f,   1,   1.6,  1.6);


kamehame_beam2 = entryEffect(SP_start+ 370 + add_f, SP_03,   0, -1,  0,  0,  0);   -- 迫るかめはめ波
setEffScaleKey(SP_start+370 + add_f, kamehame_beam2, 1, 1);
setEffScaleKey(SP_start+452 + add_f, kamehame_beam2, 1, 1);
setEffScaleKey(SP_start+454 + add_f, kamehame_beam2, 2.5, 2.5);
setDamage(SP_start+ 452 + add_f, 1, 0);  -- ダメージ振動等

-- 敵吹っ飛ぶモーション
changeAnime(SP_start+ 455 + add_f, 1, 108);
setMoveKey(SP_start+  455 + add_f, 1,  120,    0,  0);
setMoveKey(SP_start+  469 + add_f, 1,  400,    0,  0);
setScaleKey(SP_start+ 455 + add_f, 1,  1.6, 1.6);
setScaleKey(SP_start+ 458 + add_f, 1,  1.5, 1.5);
setScaleKey(SP_start+ 468 + add_f, 1,  0.2, 0.2);

-- ** エフェクト等 ** --
setShakeChara(SP_start+ 370 + add_f, 1, 99, 20);

entryFadeBg(SP_start+ 370 + add_f, 0, 99, 0,0, 0, 0, 255);          -- ベース暗め　背景

entryEffectLife(SP_start+ 370 + add_f, 921, 99, 0x80,  -1,  0,  0,  0); -- 流線斜め

-- 書き文字エントリー
ct = entryEffectLife(SP_start+ 370 + add_f, 10014, 99, 0, -1, 0, -100, 255); -- ズドドッ
setEffShake(SP_start+370 + add_f, ct, 99, 20);
setEffScaleKey(SP_start+ 370 + add_f, ct, 2.4, 2.4);
setEffRotateKey(SP_start+370 + add_f, ct, -70);
setEffAlphaKey(SP_start+370 + add_f, ct, 255);
setEffAlphaKey(SP_start+430 + add_f, ct, 255);
setEffAlphaKey(SP_start+450 + add_f, ct, 0);

entryFade(SP_start+ 452 + add_f, 3,  5, 4, fcolor_r, fcolor_g, fcolor_b, 255);     -- white fade
...
SP_dodge = 315 + add_f; --エンドフェイズのフレーム数を置き換える
...
playSe(SP_start+ 390 + add_f, SE_06);

playSe(SP_start+ 455 + add_f, SE_09);

removeAllEffect(SP_start+ 456 + add_f);

spep_4 = SP_start+ 457 + add_f;
