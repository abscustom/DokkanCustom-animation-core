--1032020:SSR_ベジータ(幼年期)_必殺技：ギャリックビーム
--sp_effect_a3_00006
--sp0027
...
entryFade( 62, 4, 5, 8, fcolor_r, fcolor_g, fcolor_b, 255);     -- white fade
...
entryFade( 170 + 80, 6, 10, 0, fcolor_r, fcolor_g, fcolor_b, 255 );  --white fade
...
setMoveKey(  259 + add_f,    0,      0,  0,   0);
setMoveKey(  260 + add_f,    0,   -600,  0,   0);
setMoveKey(  261 + add_f,    0,   -600,  0,   0);
setMoveKey(  270 + add_f,    0,   -100,  0,   0);

kamehame_beam = entryEffectLife( 260 + add_f, SP_02, 109, 0x40,  0,  300,  0,  0);   -- 伸びるかめはめ波
...
-- 途中からカメハメハが伸びる
setMoveKey(  299 + add_f,    0,   -100,  0,   0);
setMoveKey(  300 + add_f,    0,   -100,  0,   0);
setMoveKey(  308 + add_f,    0,   -1100,  0,   0);

entryFade( 365 + add_f, 3, 2, 5, fcolor_r, fcolor_g, fcolor_b, 255);     -- white fade

-- ** エフェクト等 ** --
entryFadeBg( 260 + add_f, 0, 109, 0, 10, 10, 10, 180);       -- ベース暗め　背景
entryFadeBg( 260 + add_f, 0, 40, 0, 230, 230, 230, 255);    -- 最初だけ明るい　背景
entryFadeBg( 300 + add_f, 0, 2, 8, 100, 140, 250, 250);     -- 青い　背景

entryEffectLife( 260 + add_f, 920, 109, 0x80,  -1,  0,  0,  0); -- 流線
...
entryFadeBg( 370 + add_f, 0, 99, 0, 10, 10, 10, 150);          -- ベース暗め　背景
...
entryFade( 462 + add_f, 6,  7, 4, fcolor_r, fcolor_g, fcolor_b, 255);     -- white fade
...
entryEffect( 478 + add_f, 1509,  0x80,  -1,  0,  0,  0);   -- 爆発
...
-- 書き文字エントリー
ct = entryEffectLife( 485 + add_f, 10005, 100, 0, -1, 0, -50, 230); -- ガッ
setEffShake(485 + add_f, ct, 30, 10);
setEffRotateKey( 485 + add_f, ct, -40);
setEffScaleKey( 485 + add_f, ct, 4.0, 4.0);
setEffScaleKey( 486 + add_f, ct, 2.0, 2.0);
setEffScaleKey( 487 + add_f, ct, 2.6, 2.6);
setEffScaleKey( 488 + add_f, ct, 4.0, 4.0);
setEffScaleKey( 489 + add_f, ct, 2.6, 2.6);
setEffScaleKey( 490 + add_f, ct, 3.8, 3.8);
setEffScaleKey( 580 + add_f, ct, 3.8, 3.8);
setEffAlphaKey( 485 + add_f, ct, 255);
setEffAlphaKey( 575 + add_f, ct, 255);
setEffAlphaKey( 585 + add_f, ct, 0);

playSe( 473 + add_f, SE_11);
shuchusen = entryEffectLife( 473 + add_f, 906, 120, 0x00,  -1, 0,  0,  0);   -- 集中線
entryEffect( 473 + add_f, 1600,  0x00,  -1,  0,30, -130);   -- ひび割れ
...
setEffReplaceTexture( speff, 2, 0);                         -- カード差し替え
--setEffReplaceTexture( speff, 5, 4);                                  -- 技名テクスチャ差し替え
SE001X = playSe( SP_ATK_1+22, SE_12);

spname = entryEffect( SP_ATK_1+10, 1508,  0,  -1,  0,  0,  0);               -- かめはめ波文字
setEffReplaceTexture(spname, 7, 4);                                  -- 技名テクスチャ差し替え

-- ** 白背景 ** --
entryFadeBg( SP_ATK_1 + 0, 0, 94, 0, fcolor_r, fcolor_g, fcolor_b, 255 );  --白 背景

-- ** 白フェード ** --
entryFade( SP_ATK_1 + 0, 0, 2, 4, fcolor_r, fcolor_g, fcolor_b, 255 );  --white fade
entryFade( SP_ATK_1 + 80, 6, 8, 0, fcolor_r, fcolor_g, fcolor_b, 255 );  --white fade
...
SE002X = playSe(SP_ATK_2+1, SE_07);
...
SE003X = playSe( SP_ATK_2+90, SE_06);
...
entryFade( SP_ATK_2 + 87, 3, 2, 5, fcolor_r, fcolor_g, fcolor_b, 255);     -- white fade
...
SP_dodge = 185+5 + add_fe; --エンドフェイズのフレーム数を置き換える
