...
-- ** 集中線 ** --
shuchusen = entryEffectLife( cardstart + 0, 906, 90, 0x100, -1, 0, 0, 0 );  --集中線
setEffShake( cardstart + 0, shuchusen, 90, 20 );

setEffMoveKey( cardstart + 0, shuchusen, 0, 0 , 0 );
setEffMoveKey( cardstart + 90, shuchusen, 0, 0 , 0 );

setEffScaleKey( cardstart + 0, shuchusen, 1.6, 1.6 );
setEffScaleKey( cardstart + 90, shuchusen, 1.6, 1.6 );

setEffRotateKey( cardstart + 0, shuchusen, 0 );
setEffRotateKey( cardstart + 90, shuchusen, 0 );

setEffAlphaKey( cardstart + 0, shuchusen, 255 );
setEffAlphaKey( cardstart + 90, shuchusen, 255 );
...
playSe( cardstart, SE_05);
speff = entryEffect( cardstart, 1120, 0, -1, 0, 0, 0); -- カード
setEffReplaceTexture( speff, 1, 1);
setEffReplaceTexture( speff, 2, 0); -- カード差し替え
setEffReplaceTexture( speff, 5, 4); -- 技名テクスチャ差し替え
...
playSe( cardstart, SE_05);
speff = entryEffect( cardstart, 1121, 0, -1, 0, 0, 0); -- カード
setEffReplaceTexture( speff, 1, 1);
setEffReplaceTexture( speff, 2, 0); -- カード差し替え
setEffReplaceTexture( speff, 5, 4); -- 技名テクスチャ差し替え
end
...
-- ** 次の準備 ** --
entryFade( cardstart + 82, 6, 6, 0, fcolor_r, fcolor_g, fcolor_b, 255 );  --white fade
------------------------------------------------------
--放出	ef_003	120
------------------------------------------------------
houstart = 356;
