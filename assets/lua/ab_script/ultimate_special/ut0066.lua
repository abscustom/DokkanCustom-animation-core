-- 1028570:UR_フリーザ(第一形態)_アクティブ必殺：この宇宙で最高のショー
-- sp_effect_a1_00428
-- ut0066
...
if (_IS_PLAYER_SIDE_ == 1) then
    if (_IS_SKIP_ == 1) then
       ...
       if(_IS_DODGE_ == 1) then
           skipFrame(0, spep_0 + 384 -13);   -- スキップかつ回避された時のスキップ先フレーム指定
           setupMovie(spep_0 + 384 -13, SP_01, spep_0 + 384 -13 + 2, 1);
       else
           skipFrame(0, spep_0 + 1140);  -- スキップ先フレーム指定
           setupMovie(spep_0 + 1140, SP_01, spep_0 + 1140 -1 + 2, 1);
...
-- ** エフェクト等 ** --
base_0f = entryEffect( spep_0 + 0, SP_01, 0x100, -1, 0, 0, 0); -- フリーザ　攻撃（一連）表面(ef_001)
...
changeAnime( spep_0 + 370 + OFFSET_X, 1, 102 );
changeAnime( spep_0 + 420 + OFFSET_X, 1, 118 );
changeAnime( spep_0 + 490 + OFFSET_X, 1, 108 );
...
changeAnime( spep_0 + 660 + OFFSET_X, 1, 5 );
...
SE034 = playSeVer2( spep_0 + 1214, 1128, "", 0, 0, 0, -1);
