-- 1028960: LR_セルマックス_アクティブ必殺技：スクリームエクスプロージョン
-- sp_effect_a2_00240
-- ut0072
...
SP_01b = 162958;  -- 雄叫びを上げるセルマックス～フィニッシュ ef_001b
...
if (_IS_PLAYER_SIDE_ == 1) then
    if (_IS_SKIP_ == 1) then
        ...
        if(_IS_DODGE_ == 1) then
            skipFrame(0, spep_0 + 758 -13);   -- スキップかつ回避された時のスキップ先フレーム指定
            setupMovie(spep_0 + 758 -13, SP_01, spep_0 + 758 -13 + 2, 1);
        else
            skipFrame(0, spep_0 + 850 );  -- スキップ先フレーム指定
            setupMovie(spep_0 + 850 , SP_01, spep_0 + 850 -1 + 2, 1);
        end
    else
...
-- ** エフェクト等 ** --
base_0f = entryEffect( spep_0 + 0, SP_01, 0x100, -1, 0, 0, 0); -- 雄叫びを上げるセルマックス～フィニッシュ(ef_001)
...
--[[
base_0b = entryEffect( spep_0 + 0, SP_01b, 0x80, -1, 0, 0, 0); -- 雄叫びを上げるセルマックス～フィニッシュ(ef_001b)
...
changeAnime( spep_0 + 180 + OFFSET_X, 1, 100 );
...
changeAnime( spep_0 + 800 + OFFSET_X, 1, 104 );
changeAnime( spep_0 + 836 + OFFSET_X, 1, 108 );
...
endPhase( spep_0 + MAX_FRAME_0); -- 終了フレーム 1188F
